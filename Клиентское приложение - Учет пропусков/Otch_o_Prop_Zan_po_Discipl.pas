...
OE1.Cells[1,1].Value:='За период'+' '+DateToStr(DateTimePicker1.Date)+' по '+' '
+DateToStr(DateTimePicker2.Date);
OE1.Cells[2,1].Value:='№ Студента';
OE1.Cells[2,2].Value:='ФИО Студента';
OE1.Cells[2,3].Value:='Дата пропуска';
OE1.Cells[2,4].Value:='Кол-во часов';
OE1.Cells[2,5].Value:='Причина';
istr:=3; // Номер
...
OE1.Range['C6:F'+IntToStr(istr+1)].Select; // Выделить ячейку 1 столбца 1 строки
...
OE1.Range['A1:I'+IntToStr(istr+1)].Select; // Выделить ячейки 
...
writeln(f,'<th>'+'Дата пропуска'+'</th>');
writeln(f,'<th>'+'Кол-во часов'+'</th>');
writeln(f,'<th>'+'Причина'+'</th>');
...
writeln(f,'</html>');
closefile(f);
shellexecute(Handle,'open',Pchar(extractfiledir(Application.ExeName)+'\Отчет о пропусках занятий по дисциплине.html'),nil,nil,SW_SHOWNORMAL);
...
DBGrid1.Columns.Items[2].Title.Caption :='Дата пропуска';
DBGrid1.Columns.Items[2].Width :=70;
DBGrid1.Columns.Items[3].Title.Caption :='Кол-во часов';
