...
OE1.Cells[2,1].Value:='Номер студента';
...
//OE1.Cells[istr,1].Value:=fdm.ADOQueryOtch.FieldByName('Номер_студента').AsString;
...
writeln(f,'<p></p>');
writeln(f,'<table>');
writeln(f,'<th>'+'Номер Студента'+'</th>');
...
while not fdm.ADOQueryOtch.Eof do
  begin
writeln(f,'<tr>');
writeln(f,'<td>'+fdm.ADOQueryOtch.FieldByName('Номер_студента').AsString+ '</td>');
...
writeln(f,'<td>'+fdm.ADOQueryOtch.FieldByName('Раз.администрации').AsString+ '</td>');
