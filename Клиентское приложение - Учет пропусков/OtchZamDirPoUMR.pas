...
OE1.Cells[1,1].Value:='Отчет о пропусках занятий студентом'+
'За период'+' '+'c'+' '+DateToStr(DateTimePicker1.Date)+' '+'по'+' '+DateToStr(DateTimePicker2.Date);
OE1.Cells[2,1].Value:='Дата';
OE1.Cells[2,2].value:='Пропущено часов';
OE1.Cells[2,3].Value:='Преподаватель';
OE1.Cells[3,3].Value:='Дисциплина';
OE1.Cells[4,3].Value:='Всего пропущено часов';
...
OE1.Range['A3:C'+IntToStr(istr+1)].Select;   // Выделить ячейки
...
//запустить файл
 ...
 assignfile(f,extractfiledir(Application.ExeName)+'\отчет о пропусках студентом за период.html');
ReWrite(f);
writeln(f,'<html>');
writeln(f,'<head>');
...
writeln(f,'<p></p>');
writeln(f,'<table>');
writeln(f,'<th>'+'Дата'+'</th>')  ;
writeln(f,'<th>'+'Пропущено часов'+'</th>');
writeln(f,'<th>'+'Преподаватель'+'</th>');
writeln(f,'<th>'+'Дисциплина'+'</th>');
writeln(f,'<th>'+'Всего пропущено часов'+'</th>');
FDM.ADOQueryOtch.First;               //Перейти на первую запись
...
writeln(f,'</html>');
closefile(f);
shellexecute(Handle,'open',Pchar(extractfiledir(Application.ExeName)+'\отчет о пропусках студентом за период.html'),nil,nil,SW_SHOWNORMAL);
...
FDM.ADOQueryOtch.Open;
 ...
 DBGrid1.Columns.Items[2].Title.Caption :='Преподаватель';
 DBGrid1.Columns.Items[3].Title.Caption :='Дисциплина';
 DBGrid1.Columns.Items[4].Title.Caption :='Всего часов';
end;
