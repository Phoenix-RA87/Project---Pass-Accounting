...
OE1.Cells[2,2].Value:='ФИО студент';
...
OE1.Cells[istr,2].Value:=fdm.ADOQueryOtch.FieldByName('ФИО_студент').AsString;
...
//запустить файл
//
// assignfile(f,extractfiledir(Application.ExeName)+'\Сводный отчет о пропусках занятий по дисциплине.html');
//ReWrite(f);
//writeln(f,'<html>');
//writeln(f,'<head>');
...
//writeln(f,'</html>');
//closefile(f);
//shellexecute(Handle,'open',Pchar(extractfiledir(Application.ExeName)+'\Сводный отчет о пропусках занятий по дисциплине.html'),nil,nil,SW_SHOWNORMAL);
