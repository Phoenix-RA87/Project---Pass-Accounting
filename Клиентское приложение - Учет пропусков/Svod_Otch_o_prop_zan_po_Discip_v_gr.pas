...
OE1.Cells[2,4].Value:='Наименование дисциплины';
OE1.Cells[2,5].Value:='Всего';
...
OE1.Cells[istr,1].Value:=fdm.ADOQueryOtch.FieldByName('№_п/п').AsString;
...
OE1.Cells[istr,4].Value:=fdm.ADOQueryOtch.FieldByName('Наименование_дисциплины').AsString;
OE1.Cells[istr,5].Value:=fdm.ADOQueryOtch.FieldByName('Всего').AsString;
...
writeln(f,'<th>'+'Наименование дисциплины'+'</th>');
writeln(f,'<th>'+'Всего'+'</th>');
fdm.ADOQueryOtch.First;               //Перейти на первую запись
while not fdm.ADOQueryOtch.Eof do
  begin
writeln(f,'<tr>');
writeln(f,'<td>'+fdm.ADOQueryOtch.FieldByName('№_п/п').AsString+ '</td>');
...
writeln(f,'<td>'+fdm.ADOQueryOtch.FieldByName('Наименование_дисциплины').AsString+ '</td>');
writeln(f,'<td>'+fdm.ADOQueryOtch.FieldByName('Всего').AsString+ '</td>');;
writeln(f,'</tr>');
FDM.ADOQueryOtch.Next;
end;
writeln(f,'</table>');
...
writeln(f,'</html>');
closefile(f);
shellexecute(Handle,'open',Pchar(extractfiledir(Application.ExeName)+'\Сводный отчет о пропусках занятий по подисциплине в группе.html'),nil,nil,SW_SHOWNORMAL);
...
// DBGrid1.Columns.Items[1].Title.Caption :='Пропущено часов';
// DBGrid1.Columns.Items[2].Title.Caption :='Наименование дисциплины';
// DBGrid1.Columns.Items[3].Title.Caption :='Всего';
end;
